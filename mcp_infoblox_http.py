...
# Configure logging - write to stderr only (MCP STDIO requirement)
logging.basicConfig(
    level=logging.INFO,
...
def validate_cidr(cidr: str) -> tuple[bool, str]:
    ...
    if "/" not in cidr:
        return False, "address must be in CIDR notation (e.g., 192.168.1.0/24)"
    parts = cidr.split("/")
    if len(parts) != 2:
    ...
    try:
        ...
        return False, "CIDR prefix must be a valid integer"
    return True, ""


def validate_string_length(value: str, min_len: int = 1, max_len: int = 255, field_name: str = "field") -> tuple[bool, str]:
...
try:
    ...
    logger.warning(f"InfobloxClient initialization failed: {e}. Set INFOBLOX_API_KEY environment variable to enable this feature.")
    client = None

# Initialize NIOSXaaS client (same API key as DDI)
try:
    niosxaas_client = NIOSXaaSClient()
    logger.info("NIOSXaaSClient initialized successfully")
except ValueError as e:
    logger.warning(f"NIOSXaaSClient initialization failed: {e}. Set INFOBLOX_API_KEY environment variable to enable NIOSXaaS features.")
...
try:
    ...
    logger.warning(f"AtcfwClient initialization failed: {e}. Set INFOBLOX_API_KEY environment variable to enable Atcfw/DFP features.")
    atcfw_client = None
...
try:
    ...
    logger.warning(f"InsightsClient initialization failed: {e}. Set INFOBLOX_API_KEY environment variable to enable SOC Insights features.")
...
def list_ip_spaces(name_filter: Optional[str] = None, limit: int = 100) -> dict:
    ...
    Args:
        name_filter: Filter by name (e.g., "corp" to find names containing "corp"), max 255 chars
    ...
    # Validate inputs
    valid, error_msg = validate_limit(limit, max_limit=1000)
    if not valid:
        return {"error": error_msg}

    if name_filter is not None:
        valid, error_msg = validate_string_length(name_filter, min_len=1, max_len=255, field_name="name_filter")
        if not valid:
            return {"error": error_msg}
    ...
    try:
        ...
        logger.error(f"Error listing IP spaces: {e}")
        return {"error": str(e)}
...
def list_subnets(
    ...
    Args:
        space_filter: Filter by IP space ID, max 255 chars
        address_filter: Filter by address CIDR (e.g., "192.168.0.0/16"), valid CIDR required
        limit: Maximum number of results (default: 100, max: 1000)
    ...
    if space_filter is not None:
        valid, error_msg = validate_string_length(space_filter, min_len=1, max_len=255, field_name="space_filter")
        if not valid:
            return {"error": error_msg}

    if address_filter is not None:
        valid, error_msg = validate_cidr(address_filter)
        if not valid:
            return {"error": error_msg}
...
def create_subnet(
    ...
    Args:
        address: Network address in CIDR notation (e.g., "192.168.1.0/24"), must be valid CIDR
        space: IP space ID where subnet will be created, max 255 chars
        comment: Optional description/comment for the subnet, max 255 chars
        dhcp_host: Optional DHCP host ID to serve this subnet, max 255 chars
    ...
    # Validate inputs
    valid, error_msg = validate_cidr(address)
    if not valid:
        return {"error": error_msg}

    valid, error_msg = validate_string_length(space, min_len=1, max_len=255, field_name="space")
    if not valid:
        return {"error": error_msg}

    if comment is not None:
        valid, error_msg = validate_string_length(comment, min_len=1, max_len=255, field_name="comment")
        if not valid:
            return {"error": error_msg}

    if dhcp_host is not None:
        valid, error_msg = validate_string_length(dhcp_host, min_len=1, max_len=255, field_name="dhcp_host")
        if not valid:
            return {"error": error_msg}
    ...
    try:
        ...
        logger.info(f"Created subnet {address} in space {space}")
        return result
    except Exception as e:
        logger.error(f"Error creating subnet {address}: {e}")
        return {"error": str(e)}
...
Please provide ALL these details to create a complete, functional VPN infrastructure."""
    ...
    try:
        import json
        logger.debug(f"Calling consolidated_configure with payload: {json.dumps(vpn_payload, indent=2)}")
        result = niosxaas_client.consolidated_configure(vpn_payload)
        logger.info(f"VPN infrastructure configuration succeeded")
        return result
    except Exception as e:
        logger.error(f"VPN infrastructure configuration failed: {type(e).__name__}: {str(e)}")
    ...
    try:
        ...
        if not results:
            ...
            if results:
                ...
                logger.info(f"Found service with name '{actual_name}' (case-insensitive match for '{service_name}')")
        ...
        logger.info(f"Deleting VPN service '{service_name}' (ID: {service_id})")

        # Step 3: Delete the service
        result = niosxaas_client.delete_universal_service(service_id)

        logger.info(f"Successfully deleted VPN service '{service_name}'")
        return {
            "success": True,
        ...
        logger.error(f"VPN service deletion failed: {type(e).__name__}: {str(e)}")
        return {
...
if __name__ == "__main__":
    ...
    print("🚀 Starting Infoblox BloxOne DDI MCP Server (HTTP Transport)")
    print("📍 Endpoint: http://127.0.0.1:4001/mcp")
    print("✅ Spec-compliant streamable HTTP transport")
    print("🔄 Backup SSE version still running on port 3001")
    print("🛠️  98 tools across IPAM, DNS, DHCP, VPN, Security, and SOC Insights")
